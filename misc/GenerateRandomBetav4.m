function [beta_i, distances] = GenerateRandomBetav4(L,K,cellRadius,cellHole,sshadow,gamma)
...
for n=0:1:L-1
    ...
    % Generate a random radius value uniformely distributed within the range cellHole to cellRadius for each one of the terminals.
    radius = cellHole + (cellRadius-cellHole).*rand(1,K);
    
    % Generate an random angle value uniformely distributed within the range 0 to 2*pi for each one of the terminals.
    angle = 2*pi*rand(1,K);
